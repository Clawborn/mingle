...
import { useEventData, type ParticipantWithColor } from "@/lib/useEventData";
import { mockConversations } from "@/lib/mockData";

type Message = { from: "A" | "B"; text: string };
type ConvState = {
  id: string;
  agentA: ParticipantWithColor;
  agentB: ParticipantWithColor;
  messages: Message[];
  visibleCount: number;
  status: string;
  matchReason: string;
    ...
    <div className={`rounded-2xl border transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-4 bg-white/5 flex items-center justify-between">
        ...
        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${
          isDone && conv.recommendation ? "bg-green-500/20 text-green-400"
            : isActive ? "bg-violet-500/20 text-violet-400" : "bg-white/10 text-white/40"
        }`}>
      ...
      <div className="p-4 space-y-3 max-h-64 overflow-y-auto">
          ...
          return (
            <div key={i} className="flex items-start gap-2" style={{ animation: "fadeInUp 0.3s ease-out" }}>
              ...
              <div className="flex-1">
                <div className="text-xs text-white/30 mb-1">{agent.agent_name || agent.name}</div>
                <div className="bg-white/5 rounded-xl rounded-tl-none px-3 py-2 text-sm text-white/80 leading-relaxed">{msg.text}</div>
              </div>
            </div>
          );
        })}
        {isTyping && (
          <div className="flex items-start gap-2" style={{ animation: "fadeInUp 0.3s ease-out" }}>
            ...
            <div className="bg-white/5 rounded-xl rounded-tl-none"><TypingIndicator /></div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
...
export default function LivePage() {
  ...
  const { participants, loading } = useEventData(eventId);

  // Use real participants for demo conversations when available
  const demoParticipants = participants.length >= 2 ? participants : [];
  ...
  // Initialize conversations from mock data, mapping to real participants if available
  useEffect(() => {
    if (loading) return;
    const states = mockConversations.map((c, ci) => {
      // Try to use real participants, fall back to cycling through available ones
      const aIdx = ci * 2;
      const bIdx = ci * 2 + 1;
      const agentA = demoParticipants[aIdx % demoParticipants.length] || demoParticipants[0];
      const agentB = demoParticipants[bIdx % demoParticipants.length] || demoParticipants[1];
      return {
        ...c,
        agentA: agentA || { id: c.id + "-a", name: "Agent A", avatar: "🤖", agentColor: "from-violet-500 to-purple-600", bio: "", interests: [], looking_for: null, agent_name: null, socials: {}, joined: true, event_id: eventId, created_at: "" },
        agentB: agentB || { id: c.id + "-b", name: "Agent B", avatar: "🤖", agentColor: "from-cyan-500 to-blue-600", bio: "", interests: [], looking_for: null, agent_name: null, socials: {}, joined: true, event_id: eventId, created_at: "" },
  ...
  useEffect(() => {
    if (!started || convStates.length === 0) return;
    const delays: NodeJS.Timeout[] = [];
    let totalDelay = 800;

    convStates.forEach((conv, ci) => {
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-20 pb-24">
        <div className="grid grid-cols-3 gap-4 mb-8 mt-4">
          {[
            { label: "活跃 Agent", value: participants.length, icon: "🤖" },
        ...
        {!started ? (
          <div className="text-center py-16">
            ...
            <p className="text-white/30 text-sm mb-10">{participants.length} 个 Agent 已就绪</p>
